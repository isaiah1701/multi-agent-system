...
Helmfile bootstraps only Argo CD and the AWS Load Balancer Controller. Argo CD then owns all long-lived platform and workload releases:

- External Secrets Operator (ESO), cert-manager, and ExternalDNS, each from its pinned upstream Helm chart and authenticated through EKS Pod Identity where required.
- Langfuse, from the pinned official Helm chart.
- The API, orchestrator, retriever, and answer services, from the local [`serving/helm`](../serving/helm) chart.

`serving/helm` contains the whole Helm configuration: `templates/` contains the agent, ESO custom-resource, and Argo CD application templates, while `values/` contains the bootstrap, base, and production values. `helmfile sync` installs the bootstrap control plane, then renders the chart in bootstrap mode to create the Argo CD `Application` resources for the platform controllers, Langfuse runtime secret, Langfuse, and KubeMind services. Before applying, replace `https://github.com/ORG/REPOSITORY.git` in `helmfile.yaml` with the HTTPS clone URL that Argo CD can read. For a private repository, also configure Argo CD repository credentials before the applications are reconciled.
...
4. Update the repository URL in `helmfile.yaml`, point Helm at the EKS context, and run `helmfile sync`. Helmfile installs Argo CD, which then reconciles ESO, cert-manager, ExternalDNS, and the KubeMind applications. The dedicated Langfuse runtime-secret application creates its `ClusterSecretStore` and `ExternalSecret` after ESO is available.
