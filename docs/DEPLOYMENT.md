...
4. Update the repository URL in `helmfile.yaml`, point Helm at the EKS context, and run `helmfile apply`. This installs ESO before Argo CD; Argo CD then creates the `ClusterSecretStore` and `ExternalSecret` before the application Deployments.
