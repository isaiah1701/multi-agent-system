"""Small helpers for using LangGraph message state as conversational context."""
...
from llm.client import cached_text_block


MAX_HISTORY_MESSAGES = 6
_FOLLOW_UP_MARKERS = frozenset({"it", "this", "that", "one", "ones", "they", "them", "there", "then", "also", "instead"})
_FOLLOW_UP_PREFIX = re.compile(r"^(?:what|when|why|how|would|should|can|could|do|does|is|are)\b", re.IGNORECASE)
...
def prompt_with_history(history: str, current_content: str) -> str | list[dict[str, object]]:
...
def is_contextual_kubernetes_follow_up(question: str, state: Mapping[str, Any]) -> bool:
    ...
    if not previous.allowed:
        return False
    return bool(set(words) & _FOLLOW_UP_MARKERS) or bool(_FOLLOW_UP_PREFIX.match(normalized_question))
